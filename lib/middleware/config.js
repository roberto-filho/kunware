...
function configRouter(options) {
  ...
  router.use(injectConfig(options));
  return router;
}
 ...
 * @param {Object} options The config set at the start of the application
 * @return {Function} The Express middleware injecting the configs
 */
function injectConfig(options) {
  return (req, res, next) => {
    configList.forEach((configItem) => {
      if (config[configItem.name] !== undefined || options[configItem.name] !== undefined) {
        req.headers[configItem.header] = req.headers[configItem.header] || config[configItem.name] || options[configItem.name];
      }
    });
    next();
  };
