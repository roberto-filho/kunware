...
function mock( steps, args) {
  return function( req, res, next ) {
    ...
    let useExample = ( req.headers['x-mock-example'] || args.example || 'disabled' ).toLowerCase();
    if (useExample === 'enabled' && !('example' in res.schema)) throw new Error(`No schema example found for path ${req.path}`);
    ...
    function generate( name, schema, options, gen, depth ) {
      if (useExample !== 'disabled' && 'example' in res.schema) {
