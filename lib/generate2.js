...
module.exports = {
  ...
  birthday: match(/dayofbirth|birthday/i, function(chance, schema) {
    let date = chance.birthday({type: 'adult'});
  ...
  description: match(/description$/i, function(chance, schema) {
    let textToBeReturned = chance.paragraph({sentences: 6});

    // Check for max length
    if (schema.maxLength && _.isNumber(schema.maxLength)) {
      textToBeReturned = textToBeReturned.substr(0, schema.maxLength);
    }
