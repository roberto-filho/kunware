...
function serve(args, createApp) {
  return Promise.resolve(
    ...
    app.server = app.listen(port, function() {
      ...
      if (args['config-ui'] !== false) console.log(`Config UI at http://localhost:${port}/config/ui`);
      if (args['config-back'] !== false) console.log(`Config backend at http://localhost:${port}/config`);
    });
